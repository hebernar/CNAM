...
public class FrameFacAjouter extends JFrame implements ActionListener {
	...
	public FrameFacAjouter(TableArticles61 tabArt, TableDesCommandes61 tabCde, TableFactures61 tabFact) {

		this.tabArt = tabArt;
		this.tabCde = tabCde;
		this.tabFact = tabFact;
		String[][] data = tabCde.dataNonFact();
		...
		JLabel id = new JLabel("Choisisssez la facture creer :");
		...
		else if (event.getActionCommand() == "Valider") 
		{
			...
			if (cde != null) {
				...
				if (fact == null) 
				{
						...
						fact.data(cde, tabArt);
						tabFact.ajouter(fact);
						es.affiche("Commande facturee !", new ImageIcon(this.getClass().getResource("valid-icon.png")));
						new FrameGestionTableDesFactures61(tabArt, tabCde, tabFact);
				} 
				else
				{
					es.affiche("Commande deja facturee !", new ImageIcon(this.getClass().getResource("alert-icon.png")));
